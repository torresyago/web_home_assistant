...
const ALLOWED_CERT_SERIALS = (process.env.ALLOWED_CERT_SERIALS || '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean)
  .map(normalizeSerial);
...
// aquí además comprobamos el serial contra ALLOWED_CERT_SERIALS como segunda capa.
function certAuthenticated(req) {
  ...
  if (ALLOWED_CERT_SERIALS.length > 0 && !ALLOWED_CERT_SERIALS.includes(normalizeSerial(serial))) {
    return false;
  }
  return true;
}
