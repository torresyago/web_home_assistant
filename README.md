...
- **Panel de seguridad**: desde el botón "Seguridad" del panel web puedes ver accesos válidos/fallidos (últimos 15 min, última hora e histórico), el log de los últimos accesos con fecha, IP de origen, método (contraseña, certificado o webhook) y resultado, resetear las estadísticas, gestionar manualmente qué IPs están en cuarentena (añadir o liberar), gestionar los certificados de cliente autorizados y gestionar usuarios (ver secciones siguientes). Todo se persiste en `data/db.json` y sobrevive a reinicios del contenedor. Un acceso por certificado solo se registra una vez por sesión de navegador (no en cada petición), para no saturar el log. **Solo accesible para usuarios con rol administrador.**
- **Usuarios y roles**: además del admin de arranque (`ADMIN_USER`/`ADMIN_PASSWORD`), desde la sección "Usuarios" del panel de Seguridad un administrador puede crear usuarios adicionales con contraseña propia y rol "Administrador" o "Usuario". Los usuarios con rol "Usuario" solo pueden ver y manejar los dispositivos existentes (encender, apagar, ajustar temperatura, etc.); no pueden crear ni eliminar instancias de Home Assistant ni dispositivos, ni entrar al panel de Seguridad — por tanto tampoco pueden gestionar otros usuarios, certificados o IPs en cuarentena. El rol se puede cambiar y la contraseña resetear en cualquier momento. Las contraseñas se guardan con `scrypt` (nunca en texto plano). Un certificado de cliente autenticado siempre equivale a rol administrador.
...
- **Client certificate (mTLS)**: the proxy can require a client certificate (e.g. FNMT/similar) and forward its serial number in the `X-SSL-Client-Serial` header (and optionally `X-SSL-Client-Verify`). The app validates that header against an allow-list — a double layer (proxy + app) in case the proxy's config changes. That list combines `ALLOWED_CERT_SERIALS` (optional env var) with the certificates managed from the app's own Security panel (add/remove without restarting or touching `.env`). Each certificate also has an assignable role (Admin or User, see "Users and roles" below); certificates that existed before this option default to Admin.
...
- **Security panel**: the "Seguridad" button in the web panel shows valid/failed access attempts (last 15 min, last hour, and all-time), a log of recent attempts with date, source IP, method (password, certificate, or webhook), and result, a way to reset the stats, manual management of which IPs are quarantined (add or release), management of authorized client certificates, and user management (see next points). Everything is persisted in `data/db.json` and survives container restarts. A certificate-based access is only logged once per browser session (not on every request), to avoid flooding the log. **Admin role only.**
- **Users and roles**: besides the bootstrap admin (`ADMIN_USER`/`ADMIN_PASSWORD`), from the "Users" section of the Security panel an admin can create additional users with their own password and an "Admin" or "User" role. Users with the "User" role can only view and operate existing devices (turn on/off, adjust temperature, etc.); they can't create or delete Home Assistant instances or devices, and can't reach the Security panel — so they also can't manage other users, certificates, or quarantined IPs. The role can be changed and the password reset at any time. Passwords are stored with `scrypt` (never in plain text). An authenticated client certificate always counts as admin role.
